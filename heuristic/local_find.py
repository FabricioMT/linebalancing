...
def troca_valida(graph, move, precedencias):
    ...
    if task_to in tarefa1_predecessores or task_from in tarefa2_predecessores or tarefa2_predecessores.difference(maquina1.jobs):
        return False
...
def apply_move(graph: Graph, move) -> Graph:
    ...
    #reorganizar_tarefas_e_predecessores(graph,machine_to,task_to)
    reorganizar_tarefas_e_sucessores(graph,machine_from,task_to)
    #reorganizar_tarefas_e_predecessores(graph,machine_from,task_from)
    return graph

def find_best_move(graph: Graph):
    graph.neighborhood = Neighborhood(graph)
    cpoy_swaps = graph.neighborhood.get_neighbors()
    G_copy = graph.copy()

    moves = []
    for move in cpoy_swaps:
        if troca_valida(G_copy,move,G_copy.seq):
            moves.append(move)
...
def _local_search_step(graph, copy=False):
    ...
    if best_move is not None:
        best = random.choice(best_move)
        new_graph = apply_move(new_graph,best)
